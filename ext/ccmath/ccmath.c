...
static VALUE ccmath_cos(VALUE obj, VALUE z) {
  if (f_real_p(z))
    return DBL2NUM(cos(Get_Double(z)));
...
static VALUE ccmath_sin(VALUE obj, VALUE z) {
  if (f_real_p(z))
    return DBL2NUM(sin(Get_Double(z)));
...
static VALUE ccmath_tan(VALUE obj, VALUE z) {
  if (f_real_p(z))
    return DBL2NUM(tan(Get_Double(z)));
...
static VALUE ccmath_cosh(VALUE obj, VALUE z) {
  if (f_real_p(z))
    return DBL2NUM(cosh(Get_Double(z)));
...
static VALUE ccmath_sinh(VALUE obj, VALUE z) {
  if (f_real_p(z))
    return DBL2NUM(sinh(Get_Double(z)));
...
static VALUE ccmath_exp(VALUE obj, VALUE z) {
  if (f_real_p(z))
    return DBL2NUM(exp(Get_Double(z)));
...
static double atan2_optimized(double y, double x) {
#ifndef M_PI
#define M_PI 3.14159265358979323846
  ...
  if (isinf(x) && isinf(y)) {
    ...
    double z =
        (x < 0.0) ? (3.0 * M_PI / 4.0) : (M_PI / 4.0) return (y < 0.0) ? -z : z;
  }
#endif
  return atan2(y, x);
}
