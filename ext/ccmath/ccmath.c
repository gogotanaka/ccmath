...
ccmath_cos(VALUE obj, VALUE z)
{
    if (codomain_is_real()) {
        ...
        EXTRACT_DBLS(z);
        // if (z_real == 0.0) return DBLS2COMP(cos(z_real), 0.0);
...
static VALUE
ccmath_set_codomain(VALUE obj, VALUE str)
{
...
void Init_ccmath(void)
{
    ...
    rb_define_module_function(rb_mCcmath, "codomain=", ccmath_set_codomain, 1);
}
